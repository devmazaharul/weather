...
export default function Currencyconverter() {
  ...
  useEffect(() => {
    client
      ...
      .then((response) => {
        ...
        setcurrent(Number(data[currency.to].value).toFixed(2));
      });
  }, [currency]);
  ...
  return (
    <div className="bg-gradient-to-r  from-indigo-200  to-purple-300  p-3 border w-full rounded-sm">
      <h1 className="capitalize font-bold text-center py-3">
        currency converter
      </h1>
      ...
      <div className="flex items-center gap-2 w-full mx-auto">
        <select
          ...
          value={currency.from}
          className="w-[45%] py-2 rounded-sm outline-none"
        >
        ...
        <p className="w-[5%] text-green-400">
          <FaExchangeAlt />
        </p>
        <select
